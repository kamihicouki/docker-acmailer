...
RUN \
  ...
  && curl  -sL -G --data-urlencode app_name=${VERSION} --data-urlencode 'perl_path=#!/usr/bin/perl' "http://www.acmailer.jp/cgi/install/makeinstall2.cgi" -o /var/www/html/install.cgi \
  && chmod +x /var/www/html/install.cgi
